...
Yes, the odds of men and women being wears of glasess/contact lenses for distance vision have significant difference, given the *p*-value for the test is 7.03e-05 which is less than 0.05. Based on the estimation of OR 1.24, females have 24% higher odds of wearing glasess/contact lenses for distance vision compared to men.
...
us_500_emailname <- us_500 %>%
  mutate(emailname = sub("@.*", "", email))
prop <- round(100 * sum(grepl("[^a-zA-Z0-9]", us_500_emailname$emailname)) / length(us_500_emailname$emailname), 2)
print(prop)
